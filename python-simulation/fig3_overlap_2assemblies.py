...
stimulation_strength2 = 1.15      # modulation of external input firing rate during stimulation
...
post_stimulation2_time = 250000.
post_stimulation2_step = post_stimulation2_time / post_stimulation_cicles
stimulation2_end      = post_stimulation_end + stimulation_time
post_stimulation2_end = stimulation2_end + post_stimulation2_time

# Third stimulation round
stimulation3_end = post_stimulation2_end + stimulation_time
post_stimulation3_end = stimulation3_end + post_stimulation_time

test_time = 1000.
after_test = 100000.-test_time
test_intensity = 1.1
...
nest.SetStatus(spike_detector,{'start':post_stimulation_end,'stop':post_stimulation_end+2*test_time})
...
stimulation1_steps        = np.arange(growth_time+stimulation_step, stimulation_end+1., stimulation_step)
post_stimulation1_steps   = np.arange(stimulation_end+post_stimulation_step, post_stimulation_end+1., post_stimulation_step)
stimulation2_steps        = np.arange(post_stimulation_end+stimulation_step,stimulation2_end+1., stimulation_step)
post_stimulation2_steps   = np.arange(stimulation2_end+post_stimulation_step, post_stimulation2_end+1., post_stimulation2_step)
stimulation3_steps        = np.arange(post_stimulation2_end+stimulation_step,stimulation3_end+1., stimulation_step)
post_stimulation3_steps   = np.arange(stimulation3_end+post_stimulation_step, post_stimulation3_end+1., post_stimulation_step)
...
simulate_cicle(stimulation1_steps)

# Post stimulation
nest.SetStatus([poisson_generator_ex[0]], {"rate": rate})
simulate_cicle(post_stimulation1_steps)

# Second stimulation round
nest.SetStatus([poisson_generator_ex[0]], {"rate": rate*stimulation_strength})
simulate_cicle(stimulation2_steps)

# Post stimulation
nest.SetStatus([poisson_generator_ex[0]], {"rate": rate})
simulate_cicle(post_stimulation2_steps)

# Third stimulation round
nest.SetStatus([poisson_generator_ex[1]], {"rate": rate*stimulation_strength2})
simulate_cicle(stimulation3_steps)

# Post stimulation
nest.SetStatus([poisson_generator_ex[1]], {"rate": rate})
simulate_cicle(post_stimulation3_steps)
